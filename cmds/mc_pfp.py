...
async def Cmd(message):
    try:
        ...
        templ = Image.open("cmds/image_data/Template.png")
        ...
        bge = Image.open(f"cmds/image_data/empty.png")
        if(bg == 0):
            bag = Image.open(f"cmds/image_data/empty.png")
            prop = bag.crop((0, 0, 1001, 1001))
            bge.paste(prop, (0, 0))
        if(bg != 0):
            try:
                bag = Image.open(f"cmds/image_data/bg/{bg}.png")
                prop = bag.crop((0, 0, 1001, 1001))
                bge.paste(prop, (0, 0))
            except:
                await message.channel.send("Invalid background - all backgrounds are on my support server!")
        if(starburst):
            sb = Image.open("cmds/image_data/starburst.png")
