#{}ask8ball (QUESTION) - Ask 8ball about your question...
import discord
import asyncio
...
async def Cmd(language, serverlang, message, client):
    try:
        ...
        msg8ball = await message.channel.send(language[serverlang[str(message.guild.id)]]["kernel"]["1st8ball"].replace(
            "{str(message.content).replace(config[f'prefix_manager'] + '8ball ', '')}",
            str(message.content).replace(config[f'prefix_manager'] + '8ball ', '')))
        ...
        await msg8ball.edit(content=language[serverlang[str(message.guild.id)]]["kernel"]["3nd8ball"].replace(
            "{str(message.content).replace(config[f'prefix_manager'] + '8ball ', '')}",
            str(message.content).replace(config[f'prefix_manager'] + '8ball ', '')).replace(
