#x_all_the_y - Generate X All The Y meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "x_all_the_y", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "x_all_the_y", "").split('||')[1],
   }
