#third_world_success_kid - Generate Third World Success Kid meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "third_world_success_kid", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "third_world_success_kid", "").split('||')[1],
   }
