#trump_bill_signing - Generate Trump Bill Signing meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "trump_bill_signing", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "trump_bill_signing", "").split('||')[1],
   }
