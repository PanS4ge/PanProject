#oprah_you_get_a - Generate Oprah You Get A meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "oprah_you_get_a", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "oprah_you_get_a", "").split('||')[1],
   }
