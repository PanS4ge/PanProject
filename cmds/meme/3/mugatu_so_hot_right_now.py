#mugatu_so_hot_right_now - Generate Mugatu So Hot Right Now meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "mugatu_so_hot_right_now", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "mugatu_so_hot_right_now", "").split('||')[1],
   }
