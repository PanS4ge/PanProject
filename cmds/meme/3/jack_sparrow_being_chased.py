#jack_sparrow_being_chased - Generate Jack Sparrow Being Chased meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "jack_sparrow_being_chased", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "jack_sparrow_being_chased", "").split('||')[1],
   }
