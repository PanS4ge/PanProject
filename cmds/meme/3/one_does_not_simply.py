#one_does_not_simply - Generate One Does Not Simply meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "one_does_not_simply", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "one_does_not_simply", "").split('||')[1],
   }
