#laughing_men_in_suits - Generate Laughing Men In Suits meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "laughing_men_in_suits", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "laughing_men_in_suits", "").split('||')[1],
   }
