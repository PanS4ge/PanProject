#look_at_me - Generate Look At Me meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "look_at_me", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "look_at_me", "").split('||')[1],
   }
