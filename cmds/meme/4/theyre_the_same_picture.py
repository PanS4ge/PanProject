#theyre_the_same_picture - Generate They're The Same Picture meme with 3 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "theyre_the_same_picture", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "theyre_the_same_picture", "").split('||')[1],
       'text2': str(message.content).replace(useprefix + "theyre_the_same_picture", "").split('||')[2],
   }
