#that_would_be_great - Generate That Would Be Great meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "that_would_be_great", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "that_would_be_great", "").split('||')[1],
   }
