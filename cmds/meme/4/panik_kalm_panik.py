#panik_kalm_panik - Generate Panik Kalm Panik meme with 3 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "panik_kalm_panik", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "panik_kalm_panik", "").split('||')[1],
       'text2': str(message.content).replace(useprefix + "panik_kalm_panik", "").split('||')[2],
   }
