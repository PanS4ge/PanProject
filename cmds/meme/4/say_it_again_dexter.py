#say_it_again_dexter - Generate Say it Again, Dexter meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "say_it_again_dexter", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "say_it_again_dexter", "").split('||')[1],
   }
