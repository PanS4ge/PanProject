#spongebob_ight_imma_head_out - Generate Spongebob Ight Imma Head Out meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "spongebob_ight_imma_head_out", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "spongebob_ight_imma_head_out", "").split('||')[1],
   }
