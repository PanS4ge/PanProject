#the_scroll_of_truth - Generate The Scroll Of Truth meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "the_scroll_of_truth", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "the_scroll_of_truth", "").split('||')[1],
   }
