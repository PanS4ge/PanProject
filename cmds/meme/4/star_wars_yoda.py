#star_wars_yoda - Generate Star Wars Yoda meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "star_wars_yoda", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "star_wars_yoda", "").split('||')[1],
   }
