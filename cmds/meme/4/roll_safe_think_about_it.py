#roll_safe_think_about_it - Generate Roll Safe Think About It meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "roll_safe_think_about_it", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "roll_safe_think_about_it", "").split('||')[1],
   }
