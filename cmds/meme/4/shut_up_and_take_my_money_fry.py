#shut_up_and_take_my_money_fry - Generate Shut Up And Take My Money Fry meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "shut_up_and_take_my_money_fry", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "shut_up_and_take_my_money_fry", "").split('||')[1],
   }
