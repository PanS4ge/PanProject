#drake_hotline_bling - Generate Drake Hotline Bling meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "drake_hotline_bling", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "drake_hotline_bling", "").split('||')[1],
   }
