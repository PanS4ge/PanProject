#hard_to_swallow_pills - Generate Hard To Swallow Pills meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "hard_to_swallow_pills", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "hard_to_swallow_pills", "").split('||')[1],
   }
