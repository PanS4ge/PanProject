#ill_just_wait_here - Generate I'll Just Wait Here meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "ill_just_wait_here", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "ill_just_wait_here", "").split('||')[1],
   }
