#hide_the_pain_harold - Generate Hide the Pain Harold meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "hide_the_pain_harold", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "hide_the_pain_harold", "").split('||')[1],
   }
