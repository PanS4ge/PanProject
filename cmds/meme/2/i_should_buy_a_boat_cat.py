#i_should_buy_a_boat_cat - Generate I Should Buy A Boat Cat meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "i_should_buy_a_boat_cat", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "i_should_buy_a_boat_cat", "").split('||')[1],
   }
