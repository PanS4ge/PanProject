#grandma_finds_the_internet - Generate Grandma Finds The Internet meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "grandma_finds_the_internet", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "grandma_finds_the_internet", "").split('||')[1],
   }
