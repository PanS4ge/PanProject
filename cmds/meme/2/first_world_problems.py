#first_world_problems - Generate First World Problems meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "first_world_problems", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "first_world_problems", "").split('||')[1],
   }
