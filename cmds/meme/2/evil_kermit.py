#evil_kermit - Generate Evil Kermit meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "evil_kermit", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "evil_kermit", "").split('||')[1],
   }
