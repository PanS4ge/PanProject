#grus_plan - Generate Gru's Plan meme with 4 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "grus_plan", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "grus_plan", "").split('||')[1],
       'text2': str(message.content).replace(useprefix + "grus_plan", "").split('||')[2],
       'text3': str(message.content).replace(useprefix + "grus_plan", "").split('||')[3],
   }
