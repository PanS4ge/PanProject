#guy_holding_cardboard_sign - Generate Guy Holding Cardboard Sign meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "guy_holding_cardboard_sign", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "guy_holding_cardboard_sign", "").split('||')[1],
   }
