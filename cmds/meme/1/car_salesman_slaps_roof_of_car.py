#car_salesman_slaps_roof_of_car - Generate Car Salesman Slaps Roof Of Car meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "car_salesman_slaps_roof_of_car", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "car_salesman_slaps_roof_of_car", "").split('||')[1],
   }
