#cute_cat - Generate Cute Cat meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "cute_cat", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "cute_cat", "").split('||')[1],
   }
