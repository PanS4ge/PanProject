#batman_slapping_robin - Generate Batman Slapping Robin meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "batman_slapping_robin", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "batman_slapping_robin", "").split('||')[1],
   }
