#brace_yourselves_x_is_coming - Generate Brace Yourselves X is Coming meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "brace_yourselves_x_is_coming", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "brace_yourselves_x_is_coming", "").split('||')[1],
   }
