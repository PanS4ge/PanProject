#bike_fall - Generate Bike Fall meme with 3 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "bike_fall", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "bike_fall", "").split('||')[1],
       'text2': str(message.content).replace(useprefix + "bike_fall", "").split('||')[2],
   }
