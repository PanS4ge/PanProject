#change_my_mind - Generate Change My Mind meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "change_my_mind", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "change_my_mind", "").split('||')[1],
   }
