#but_thats_none_of_my_business - Generate But That's None Of My Business meme with 2 boxes.
import discord
import requests
import json
username = "rng31323"
...
async def Cmd(language, serverlang, message, client):
   ...
   params = {
       ...
       'text0': str(message.content).replace(useprefix + "but_thats_none_of_my_business", "").split('||')[0],
       'text1': str(message.content).replace(useprefix + "but_thats_none_of_my_business", "").split('||')[1],
   }
