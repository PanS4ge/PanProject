...
def Eval_Test_If_Have(ide, towho):
    badge = {}
    with open(f"badges.json", "r") as cny:
        badge = json.loads(cny.read())
        cny.close()
    print("Testing badge " + badge['badge'][ide]['name'])
    if(Have_Badge(ide, towho)):
        print("yess " + str(Have_Badge(ide, towho)))
    else:
        print("nooo " + str(Have_Badge(ide, towho)))
